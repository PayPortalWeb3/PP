...
- **$PP Token Integration**: Native support for $PP token (token mint address to be configured)
  - SPL token payment verification
  - Holder discount tiers (Bronze 5% → Diamond 50%)
  - Payment discount when using $PP token
  - `PortalTokenManager` class for balance checks and discounts
  - Helper functions: `isPortalToken()`, `formatPortalAmount()`

### New Exports

- `PORTAL_TOKEN` - Token constants (mint address, decimals, symbol)
- `DEFAULT_DISCOUNT_TIERS` - Default holder discount tiers
- `PortalTokenManager` - Token management class
- `createPortalTokenManager()` - Factory function
- `PortalTokenConfig` type
